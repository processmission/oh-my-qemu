...
- Read the domain skill named by the workstream before changing source:
  - `peripheral-modeling` -> `skill://qemu-workflow-peripheral-modeling`
  - `board-modeling` -> `skill://qemu-workflow-board-modeling`
  - `tcg-frontend` -> `skill://qemu-workflow-tcg-frontend-instruction`
  - `tcg-backend` -> `skill://qemu-workflow-tcg-backend-adaptation`

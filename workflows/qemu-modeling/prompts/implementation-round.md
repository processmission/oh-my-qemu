...
2. Make the smallest local QEMU source, docs, build, debug, image, or boot changes needed by the selected workstream.
3. Use existing QEMU conventions; do not introduce parallel abstractions when an in-tree pattern exists.
4. Update `{{taskRoot}}/commands.md` with exact commands that should be run or were run during this implementation node.
5. Update `{{taskRoot}}/evidence.md` and `{{taskRoot}}/rlcr/goal-tracker.md` with changed files, decisions, and remaining gaps.

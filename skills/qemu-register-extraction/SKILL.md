...
7. **Record the framework decision.** For every guest-visible control/status
   register bank, record the applicable project policy, explicit user
   direction, and nearby subsystem convention. Select `RegisterInfo` or
   justified manual MMIO callbacks from that evidence, and keep register
   offsets, field macros, backing storage, framework tables, and register-local
   hooks in the device `.c` file, not a header.

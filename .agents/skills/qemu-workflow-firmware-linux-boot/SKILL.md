---
name: qmu-workflow-firmware-linux-boot
...
1. `qemu-plan` to freeze scope, artifact root, and acceptance criteria.
2. `qemu-source-provenance` for firmware, bootloader, kernel, DTB,
   rootfs, and QEMU inputs.
3. `qemu-image-layout` for boot media, pflash, ROM, or disk images.
4. `qemu-boot-run` to run and log the firmware path.
5. `qemu-debug` to isolate the failing stage if boot stalls.
6. `qemu-model-verification` to report the reached milestone.
7. `qemu-rlcr-loop` for staged implementation or debugging rounds when
   source or boot artifact changes are required.

This workflow also owns the firmware-to-Linux boot path contract:
